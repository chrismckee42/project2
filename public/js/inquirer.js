...
const listChoices = {
  //key is the message. value is the choices
  "start game": ["start"],
  "choose role": [
    "Coward (+% flee chance)",
    "Barbara the Wall (+pts hp)",
    "Jabby McJabberson (+pts attack)"
  ],
  "move directions": directionObj =>
    Object.keys(directionObj).map(direction => ({
      // directionObj = {north: true, south: true, east: false, west: true}
      name: direction,
...
const promptList = {
  list: (key, choices, name, message) => ({
    // directionObj = {north: true, south: true, east: false, west: true}...
    type: "list",
    choices:
      choices && !Array.isArray(listChoices[key])
        ? listChoices[key](choices)
        : listChoices[key],
  ...
  "order qty": (qty, name) => ({
    ...
    validate: input =>
      !isNaN(input) && qty >= input
        ? true
        : `The number you entered is invalid. Please pick a number less than or equal to ${qty}.`
  })
};
...
const init = async () => {
  ...
  const dealWithMonster = async () => {
    ...
    if (!monster) {
      console.log("there are no monsters present");
    } else {
      console.log("A monster is present, he charges you, what do you do?");
      const { battleOption } = await prompt(
        "battle options",
        null,
        "battleOption"
      );
